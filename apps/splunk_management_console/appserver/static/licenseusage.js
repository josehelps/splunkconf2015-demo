require([
	...
	function(mvc, utils, _, $, simpleXml, config, splunkUtils, splunkdUtils, i18n, route) {
        // Manually replacing all labels with its i18n version
        ...
        $('#help_today').attr("href", route.docHelp(root, config.LOCALE, "learnmore.license.usage_today"));

        $('#help_30days').attr("href", route.docHelp(root, config.LOCALE, "learnmore.license.usage_30days"));
    	
        var BASE_SUMMARY_SEARCH = "index=_internal $HostClause$ source=*license_usage.log type=\"RolloverSummary\" earliest=-30d@d $PoolClause$";
    	var BASE_USAGE_SEARCH = "index=_internal $HostClause$ source=*license_usage.log type=\"Usage\" | eval h=if(len(h)=0 OR isnull(h),\"(SQUASHED)\",h) | eval s=if(len(s)=0 OR isnull(s),\"(SQUASHED)\",s) | eval idx=if(len(idx)=0 OR isnull(idx),\"(UNKNOWN)\",idx) | bin _time span=1d | stats sum(b) as b by _time, pool, s, st, h, idx $PoolClause$";
		var STACK_SIZE_SEARCH = splunkUtils.sprintf(" | join type=outer _time [search index=_internal $HostClause$ source=*license_usage.log type=\"RolloverSummary\" earliest=-30d@d | eval _time=_time - 43200 | bin _time span=1d | stats latest(stacksz) AS \"%s\" by _time] | fields - _timediff ", _("stack size").t());
		var POOL_SIZE_SEARCH = splunkUtils.sprintf(" | join type=outer _time [search index=_internal $HostClause$ source=*license_usage.log type=\"RolloverSummary\" earliest=-30d@d $PoolClause$ | eval _time=_time - 43200 | bin _time span=1d | stats latest(poolsz) AS  \"%s\" by _time] | fields - _timediff ", _("pool size").t());
		
		var GET_ALL_POOLS = "| rest $ServerClause$ /services/licenser/pools | rename title AS pool | search [rest $ServerClause$ /services/licenser/groups | search is_active=1 | eval stack_id=stack_ids | fields stack_id] | eval name=pool | eval value=\"pool=\\\"\". pool . \"\\\"\" | table name value";
		
        var GET_LICENSE_MASTERS = 
            '|  `dmc_get_instance_info("dmc_group_license_master")`' + '| eval group_server_host = serverName.",".host | fields title, host, serverName, group_server_host';

        var TODAY_LICENSE_USAGE = "| rest $ServerClause$ /services/licenser/pools | rename title AS Pool | search [rest $ServerClause$ /services/licenser/groups | search is_active=1 | eval stack_id=stack_ids | fields stack_id] | join type=outer stack_id [rest $ServerClause$ /services/licenser/stacks | eval stack_id=title | eval stack_quota=quota | fields stack_id stack_quota] | stats sum(used_bytes) as used max(stack_quota) as total | eval usedGB=round(used/1024/1024/1024,3) | eval totalGB=round(total/1024/1024/1024,3) | eval gauge_base=0 | eval gauge_danger=totalGB*0.8 | eval gauge_top=totalGB+0.001 | gauge usedGB gauge_base gauge_danger totalGB gauge_top";		
		var TODAY_LICENSE_USAGE_BY_POOL = splunkUtils.sprintf("| rest $ServerClause$ /services/licenser/pools | rename title AS Pool | search [rest $ServerClause$ /services/licenser/groups | search is_active=1 | eval stack_id=stack_ids | fields stack_id] | eval quota=if(isnull(effective_quota),quota,effective_quota) | eval \"%s\"=round(used_bytes/1024/1024/1024, 3) | eval \"%s\"=round(quota/1024/1024/1024, 3) | fields Pool \"%s\" \"%s\"", _("Used").t(), _("Quota").t(), _("Used").t(), _("Quota").t());
		var TODAY_LICENSE_USAGE_PCT = splunkUtils.sprintf("| rest $ServerClause$ /services/licenser/pools | rename title AS Pool | search [rest $ServerClause$ /services/licenser/groups | search is_active=1 | eval stack_id=stack_ids | fields stack_id] | eval quota=if(isnull(effective_quota),quota,effective_quota) | eval \"%s\"=round(used_bytes/quota*100,2) | fields Pool \"%s\"", _("% used").t(), _("% used").t());
		var POOL_WARNING = "| rest $ServerClause$ /services/licenser/messages | where (category==\"license_window\" OR category==\"pool_over_quota\") AND create_time >= now() - (30 * 86400) | rename pool_id AS pool | eval warning_day=if(category==\"pool_over_quota\",\"(\".strftime(create_time,\"%B %e, %Y\").\")\",strftime(create_time-43200,\"%B %e, %Y\")) | fields pool warning_day | join outer pool [rest $ServerClause$ /services/licenser/slaves | mvexpand active_pool_ids | eval slave_name=label | eval pool=active_pool_ids | fields pool slave_name | stats values(slave_name) as \"members\" by pool] | join outer pool [rest $ServerClause$ /services/licenser/pools | eval pool=title | eval quota=if(isnull(effective_quota),quota,effective_quota) | eval quotaGB=round(quota/1024/1024/1024,3) | fields pool stack_id, quotaGB] |" + splunkUtils.sprintf("stats first(pool) as \"%s\" first(stack_id) as \"%s\" first(members) as \"%s\" first(quotaGB) as \"%s\" values(warning_day) AS \"%s\" by pool | fields - pool", _("Pool").t(), _("Stack ID").t(), _("Current Members").t(), _("Current Quota (GB)").t(), _("Warning Days - (Soft)/Hard").t());	
		var SLAVE_WARNING = splunkUtils.sprintf("| rest $ServerClause$ /services/licenser/slaves | mvexpand active_pool_ids | where warning_count>0 | eval pool=active_pool_ids | join type=outer pool [rest $ServerClause$ /services/licenser/pools | eval pool=title | fields pool stack_id] | eval in_violation=if(warning_count>4 OR (warning_count>2 AND stack_id==\"free\"),\"%s\",\"%s\") | fields label, title, pool, warning_count, in_violation | fields - _timediff | rename label as \"%s\" title as \"%s\" pool as \"%s\" warning_count as \"%s\" in_violation AS \"%s\"", _("yes").t(), _("no").t(), _("Slave").t(), _("GUID").t(), _("Pool").t(), _("Hard Warnings").t(), _("In Violation?").t());
		...
		var LICENSE_USAGE_BY_INDEXER =                      BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b by slave, pool, _time | eval slave_guid=slave | stats max(b) AS volume by slave_guid, _time | join type=outer slave_guid [rest $ServerClause$ /services/licenser/slaves | rename label AS slave_name title AS slave_guid | table slave_guid slave_name] | eval slave_name = if(isnotnull(slave_name),slave_name,\"GUID: \".slave_guid) | timechart span=1d max(volume) AS \"%s\" by slave_name fixedrange=false", _("volume").t()) + STACK_SIZE_SEARCH + " | foreach * [eval <<FIELD>>=round('<<FIELD>>'/1024/1024/1024, 3)]";
		var LICENSE_USAGE_BY_INDEXER_POOL_SELECTED =        BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b by slave, pool, _time | eval slave_guid=slave | stats max(b) AS volume by slave_guid, _time | join type=outer slave_guid [rest $ServerClause$ /services/licenser/slaves | rename label AS slave_name title AS slave_guid | table slave_guid slave_name] | eval slave_name = if(isnotnull(slave_name),slave_name,\"GUID: \".slave_guid) | timechart span=1d max(volume) AS \"%s\" by slave_name fixedrange=false", _("volume").t()) + POOL_SIZE_SEARCH + " | foreach * [eval <<FIELD>>=round('<<FIELD>>'/1024/1024/1024, 3)]";
		...
		var LICENSE_USAGE_BY_INDEXER_PCT =                  BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b latest(stacksz) AS stacksz by slave, pool, _time | stats sum(b) AS volumeB max(stacksz) AS stacksz by slave, _time | eval pctused=round(volumeB/stacksz*100,2)| eval slave_guid=slave | join type=outer slave_guid [rest $ServerClause$ /services/licenser/slaves | rename label AS slave_name title AS slave_guid | table slave_guid slave_name] | eval slave_name = if(isnotnull(slave_name),slave_name,\"GUID: \".slave_guid) | timechart span=1d max(pctused) AS \"%s\" by slave_name fixedrange=false", _("% used").t());
        ...
        var ACCELERATED_SEARCH_QUERY = "|rest $ServerClause$ /services/saved/searches/License%20Usage%20Data%20Cube | fields auto_summarize";
    	...
    	var NO_SUMMARY_MSG = splunkUtils.sprintf(_("The report that powers these panels is not accelerated.  For faster performance, you might wish to turn on acceleration for this report. <a href=\"%s\" target=\"_blank\" title=\"%s\">%s</a>").t(), route.docHelp(root, config.LOCALE, "learnmore.license.usage_acceleration"), _("Splunk help").t(), _("Learn more").t());
	    ...
	    var HistoricSearches = {
	       	   "license_usage_no_split" : {
	       	   	    "pool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_NOSPLIT_POOL_SELECTED }, 
                        "nooverlay" : {
                            "dailySearch" : LICENSE_USAGE_NOSPLIT_POOL_SELECTED + REMOVE_POOL_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_NOSPLIT_PCT_POOL_SELECTED},
	       	   		"nopool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_NOSPLIT}, 
                        "nooverlay" : { 
                            "dailySearch" : LICENSE_USAGE_NOSPLIT + REMOVE_STACK_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_NOSPLIT_PCT},
	       	   		"maxAvgSearch" : LICENSE_USAGE_NOSPLIT_MAXAVG
	       	   },
	       	   "license_usage_by_pool" : {
	       	   		"pool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_POOL_POOL_SELECTED }, 
                        "nooverlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_POOL_POOL_SELECTED + REMOVE_POOL_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_BY_POOL_PCT_POOL_SELECTED},
                    "nopool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_POOL}, 
                        "nooverlay" : { 
                            "dailySearch" : LICENSE_USAGE_BY_POOL + REMOVE_STACK_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_BY_POOL_PCT},
	       	   		"maxAvgSearch" : LICENSE_USAGE_BY_POOL_MAXAVG
	       	   },
	       	   "license_usage_by_indexer" : {
	       	   		"pool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_INDEXER_POOL_SELECTED }, 
                        "nooverlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_INDEXER_POOL_SELECTED + REMOVE_POOL_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_BY_INDEXER_PCT_POOL_SELECTED},
	       	   		"nopool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_INDEXER}, 
                        "nooverlay" : { 
                            "dailySearch" : LICENSE_USAGE_BY_INDEXER + REMOVE_STACK_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_BY_INDEXER_PCT},
	       	   		"maxAvgSearch" : LICENSE_USAGE_BY_INDEXER_MAXAVG
	       	   },
	       	   "license_usage_by_sourcetype" : {
	       	   		"pool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_SOURCETYPE_POOL_SELECTED }, 
                        "nooverlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_SOURCETYPE_POOL_SELECTED + REMOVE_POOL_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_BY_SOURCETYPE_PCT_POOL_SELECTED},
	       	   		"nopool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_SOURCETYPE}, 
                        "nooverlay" : { 
                            "dailySearch" : LICENSE_USAGE_BY_SOURCETYPE + REMOVE_STACK_SIZE},
                        "pctSearch" : LICENSE_USAGE_BY_SOURCETYPE_PCT},
	       	   		"maxAvgSearch" : LICENSE_USAGE_BY_SOURCETYPE_MAXAVG
	       	   },
	       	   "license_usage_by_host" : {
	       	   		"pool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_HOST_POOL_SELECTED }, 
                        "nooverlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_HOST_POOL_SELECTED + REMOVE_POOL_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_BY_HOST_PCT_POOL_SELECTED},
	       	   		"nopool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_HOST}, 
                        "nooverlay" : { 
                            "dailySearch" : LICENSE_USAGE_BY_HOST + REMOVE_STACK_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_BY_HOST_PCT},
	       	   		"maxAvgSearch" : LICENSE_USAGE_BY_HOST_MAXAVG
	       	   },
	       	   "license_usage_by_source" : {
	       	   		"pool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_SOURCE_POOL_SELECTED }, 
                        "nooverlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_SOURCE_POOL_SELECTED + REMOVE_POOL_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_BY_SOURCE_PCT_POOL_SELECTED},
	       	   		"nopool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_SOURCE}, 
                        "nooverlay" : { 
                            "dailySearch" : LICENSE_USAGE_BY_SOURCE + REMOVE_STACK_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_BY_SOURCE_PCT},
	       	   		"maxAvgSearch" : LICENSE_USAGE_BY_SOURCE_MAXAVG
	       	   },
	       	   "license_usage_by_index" : {
	       	   		"pool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_INDEX_POOL_SELECTED }, 
                        "nooverlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_INDEX_POOL_SELECTED + REMOVE_POOL_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_BY_INDEX_PCT_POOL_SELECTED},
	       	   		"nopool" : {
                        "overlay" : {
                            "dailySearch" : LICENSE_USAGE_BY_INDEX}, 
                        "nooverlay" : { 
                            "dailySearch" : LICENSE_USAGE_BY_INDEX + REMOVE_STACK_SIZE}, 
                        "pctSearch" : LICENSE_USAGE_BY_INDEX_PCT},
		...
		// RT EDIT
        // Checks to see if usage search summary is enabled, then sets info messages.
		function updateInfoMessages(serverDropdownVal, splitDropdownVal) { 
            var server_clause = serverDropdownVal ? "splunk_server="+serverDropdownVal.split(",")[0] : ""; 
			if(!serverDropdownVal) return;
            
            checkAcceleratedSearch.set("search", ACCELERATED_SEARCH_QUERY.replace(/\$ServerClause\$/g, server_clause));
            
			isBaseUsageSearchSummarized = false;

            checkAcceleratedSearch.startSearch();
            var results = checkAcceleratedSearch.data("results");
            results.once("data", function() {
                // pops the one and only model from collection and grabs its 'accelerated search' state
                var auto_summarize = results.collection().pop().get("auto_summarize");
                
                // if state not present, something went wrong
                if(!auto_summarize) {
                    console.log("Error fetching accelerated search report.");
                } else if(auto_summarize == 1) {
        ...
        // RT ADD
        // sets the splunk_server field in the search query which corresponds to the selected dropdown server
        function setRestServer(serverDropdownVal) {
            var server_clause = serverDropdownVal ? "splunk_server="+serverDropdownVal.split(",")[0] : ""; 
            allPoolsSearch.set("search", GET_ALL_POOLS.replace(/\$ServerClause\$/g, server_clause));            
            todayLicenseUsageSearch.set("search", TODAY_LICENSE_USAGE.replace(/\$ServerClause\$/g, server_clause));
            todayLicenseUsageByPoolSearch.set("search", TODAY_LICENSE_USAGE_BY_POOL.replace(/\$ServerClause\$/g, server_clause));     
            todayLicenseUsagePctSearch.set("search", TODAY_LICENSE_USAGE_PCT.replace(/\$ServerClause\$/g, server_clause)); 
            poolWarningSearch.set("search", POOL_WARNING.replace(/\$ServerClause\$/g, server_clause));
            slaveWarningSearch.set("search", SLAVE_WARNING.replace(/\$ServerClause\$/g, server_clause));
        }
	    ...
	    function setHistoricSearches(poolDropdownVal, splitDropdownVal, serverDropdownVal) {
	        var poolSelected = poolDropdownVal.length > 1 ? true : false;
	        var toShowOverlay = isShowOverlay();
           
            // RT EDIT
            var server_clause = serverDropdownVal ? "splunk_server="+serverDropdownVal.split(",")[0] : ""; 
            var host_clause = serverDropdownVal ? "host="+serverDropdownVal.split(",")[1] : "";
		    ...
		    if (poolSelected && (
                        splitDropdownVal == "license_usage_by_sourcetype" || 
                        splitDropdownVal == "license_usage_by_host" || 
                        splitDropdownVal == "license_usage_by_source" || 
                        splitDropdownVal == "license_usage_by_index")) {
                poolDropdownVal = " | search " + poolDropdownVal;      
            }

            // RT EDIT
            var mapObj = {
                "$PoolClause$" : poolDropdownVal,
                "$ServerClause$": server_clause,
                "$HostClause$" : host_clause
            };

            dailyUsageSearch.set("search", search1.replace(/\$PoolClause\$|\$ServerClause\$|\$HostClause\$/g, function(matched) { return mapObj[matched]; }));	
            dailyPctSearch.set("search", search2.replace(/\$PoolClause\$|\$ServerClause\$|\$HostClause\$/g, function(matched) { return mapObj[matched]; }));
            maxAvgSearch.set("search", search3.replace(/\$PoolClause\$|\$ServerClause\$|\$HostClause\$/g, function(matched) { return mapObj[matched]; }));
        }
		...
		require(["contrib/bootstrap-2.3.1/bootstrap-tab"], function() {
			...
			$('#myTabs a[href=#historyTab]').click(function (e) {
				...
				// Charts disappear when switching tabs (container height is 0 for some reason),
				// so redraw manually				
				invalidateChart(chartUsage30days);
				invalidateChart(chartPctUsage30days);
				invalidateChart(chartAvgMaxUsage30days);		
			});
		});	
	    ...
	    var allPoolsSearch = new SearchManager({
	        "id": "search9",
	        "autostart": true,
	        "search": "",
        ...
        // RT ADD
        var serverDropdown = new DropdownInput({
            "id": "field4",
            "labelField": "serverName",
            "valueField": "group_server_host",
            "managerid": "search10",
            "submitOnChange": false,
            "selectFirstChoice": true,
            "token": "License_Server",
            "el": $('#field4'),
            "showClearButton": false
        }).render();

        // RT ADD
        var getLicenseMasters = new SearchManager({
            "id": "search10",
            "autostart": true,
            "search": GET_LICENSE_MASTERS,
            "auto_cancel": 90,
            "status_buckets": 0,
            "preview": false,
            "wait": 0
        ...
        var checkAcceleratedSearch = new SearchManager({
            "id": "search11",
            "autostart": false,
            "search": "",
            "auto_cancel": 90,
	    ...
	    // Create searches
	    //
	    var todayLicenseUsageSearch = new SearchManager({
	        "id": "search1",
	        "earliest_time": null,
	        "search": "",
	        "latest_time": null,
	    ...
	    var todayLicenseUsageByPoolSearch = new SearchManager({
	        "id": "search2",
	        "latest_time": null,
	        "earliest_time": null,
	        "search": "",
	    ...
	    var todayLicenseUsagePctSearch = new SearchManager({
	        "id": "search3",
	        "earliest_time": null,
	        "search": "",
	        "latest_time": null,
	    ...
	    var poolWarningSearch = new SearchManager({
	        "id": "search4",
	        "earliest_time": null,
	        "search": "",
	        "latest_time": null,
	    ...
	    var slaveWarningSearch = new SearchManager({
	        "id": "search5",
	        "earliest_time": null,
	        "search": "",
	        "latest_time": null,
        ...
        //RT ADD
        serverDropdown.on("change", function(serverDropdownVal) {
            var splitDropdownVal = splitDropdown.val();
	    	var poolDropdownVal = poolDropdown.val();

            setRestServer(serverDropdownVal);
		    updateInfoMessages(serverDropdownVal, splitDropdownVal);	
	        setHistoricSearches(poolDropdownVal, splitDropdownVal, serverDropdownVal);
        });

	    poolDropdown.on("change", function(poolDropdownVal) {
	        var splitDropdownVal = splitDropdown.val();
            var serverDropdownVal = serverDropdown.val();
		...
		showHideOverlay.on("change", function(val) {
			setHistoricSearches(poolDropdown.val(), splitDropdown.val(), serverDropdown.val()); 
		});
